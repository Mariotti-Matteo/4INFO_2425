...
int socket_send(int socket_fd, struct sockaddr_in* ip, unsigned short port, char *buf) 
{
    ...
    serveraddr.sin_addr.s_addr = ip->sin_addr.s_addr;
...
int main(int argc, char **argv) 
{
    ...
    byte_sent = socket_send(socket_fd, clientaddr, udp_port, argv[3]); 
    printf("Inviato %d bytes con successo a %s\n", byte_sent, clientaddr);
